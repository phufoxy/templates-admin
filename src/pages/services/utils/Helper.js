...
export default class Helpers {
  static tagStatus = (type) => {
    if (type === variables.STATUS.ON) {
      return (
        <Tag className={classnames(styles.tab, styles.success)}>{variables.STATUS_NAME.ON}</Tag>
      );
    }
    ...
    return <Tag className={classnames(styles.tab, styles.yellow)}>{variables.STATUS_NAME.OFF}</Tag>;
  };
}
